...
type ArrayDiff<T> = { index: number } & ({ type: 'Insert', values: T[]; } | { type: 'Remove', count: number } | { type: 'Replace', value: T, oldValue: T, oldIndex: number });
...
type ObjectDiff<T> = { key: string } & ({ type: 'Add', value: T; } | { type: 'Remove' } | { type: 'Replace', value: T, oldValue: T });
